...
lexIdentOrKeyword s =
  let (word,rest) = span isAlphaNum s
      reserved = ["while","do","String","Monad","Maybe","Just","Nothing","IO","Either","Left","Right","fmap","map"]
...
buildAssignMap [] = []
buildAssignMap (TKeyword "String": TIdentifier v : TAssign : TString s : xs) =
  (v,s) : buildAssignMap xs
buildAssignMap (TKeyword "Monad": TIdentifier v : TAssign : t : xs) =
  (v, tokenLexeme t) : buildAssignMap xs
buildAssignMap (TIdentifier v : TAssign : TReal r : xs) =
  (v, show r) : buildAssignMap xs
buildAssignMap (TIdentifier v : TAssign : TIdentifier w : xs) =
  (v, w) : buildAssignMap xs
buildAssignMap (_:xs) = buildAssignMap xs
       ...
       then Left (unlines es)
         ...
         in case noComm of
              [ TWhile, TIdentifier i1, TOp "<", TReal lim, TDo, TSemicolon
                , TIdentifier i2, TAssign, TIdentifier i3, TOp "+"
                , TReal st, TSemicolon]
                  | i1==i2 && i2==i3 ->
                      case execWhileLoop 0 lim st 0 of
                        Left e -> Left e
                        Right r-> Right ("Цикл while: "++i1++" = "++ show r)
              [ TDo, TIdentifier i1, TAssign, TIdentifier i2, TOp "+"
                , TReal st, TSemicolon
                , TWhile, TIdentifier i3, TOp "<"
                , TReal lim, TSemicolon]
                  | i1==i2 && i2==i3 ->
